...
def test_covid(dataset,location_name):
    ...
    formula = """positive ~ d_temp +d_prcp + period_group + did1 """
    ...
    formula = """recovered ~ d_temp+d_prcp  + period_group + did1 """
    response, predictors = dmatrices(formula, data, return_type='dataframe')
    po_results = sm.GLM(response, predictors, family=sm.families.Poisson()).fit()
    print(po_results.summary())
    ...
    formula = """death ~ d_temp+d_prcp  + period_group + did1 """
    response, predictors = dmatrices(formula, data, return_type='dataframe')
    po_results = sm.GLM(response, predictors, family=sm.families.Poisson()).fit()
    print(po_results.summary())
...
def runSimulator(data1,coefsdfR,sir_names,xnamesr,horizon1):
    	###DEFINE SIMULATION CONSTANTS#######################################################
	# horizon = len(date_list)  # days . - forecast days
    # data1 starts from end of historical data date minus 7 days (from training)
    data = data1.copy()
    ...
    for state in coefsR['state'].drop_duplicates():
        ...
        #try:
            ...
            if len(df)>0 and len(cr)>0:
                ...
                y = solve_one(state=state,df=df,cfr=cfr,alpha=alpha,
                
                beta=cfr['lag_confirmed'].values[0],
                c_x = c_xr,
                ...
                plt.legend(['Suceptibes','Confirmed','removed'])
                # plt.tight_layout()
                plt.savefig(os.path.join('D:\Documents\Engage\output\covid_plot_'+state+'.png'))
                plt.clf()
                # if len(out_df) ==0:
                #     out_df = df
                # else:
...
def causal_simulation(path,start_date,f_start_date,datafile="dataset_full.csv",govpolicyfile="gov_dates_mandates.csv"):
    ...
    select province_state , country_region as country, date, confirmed,
       recovered, death, population, PRCP, TAVG/10 as TAVG,
    ...
    sim_data_output = runSimulator(data1=data3[data3['holdout']==1],
    coefsdfR=r_combined,
    sir_names=['susceptible','confirmed','death','removed'],
    xnamesr=['Intercept','gov_action','TAVG','lag_confirmed'],
    horizon1=60)
    sim_data_output.to_csv(path+"/simulations.csv")
     


if __name__=="__main__":

    path="D:\Documents\Airflow\data"
    ...
    causal_simulation(path=path,start_date=start_date,f_start_date="4/20/20", 
    datafile="dataset_full.csv",govpolicyfile="gov_dates_mandates.csv")

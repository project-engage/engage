...
def rename_location(df):
	location_list = pd.read_csv("location_match.csv")
...
if __name__ == '__main__':
	...
	confirmed_covid = pd.read_csv("jhu_confirmed_covid.csv")
	confirmed_covid = confirmed_covid.loc[:,~confirmed_covid.columns.isin(EXCLUDE_COL)]
	recovered_covid = pd.read_csv("jhu_recovered_covid.csv")
	recovered_covid = recovered_covid.loc[:,~recovered_covid.columns.isin(EXCLUDE_COL)]
	death_covid = pd.read_csv("jhu_death_covid.csv")
	death_covid = death_covid.loc[:,~death_covid.columns.isin(EXCLUDE_COL)]

	us_covid = pd.read_csv("daily_covid_usstates.csv")[INCLUDE_COL].sort_values(by=['date'], ascending=True).reset_index(drop=True)
	...
	jhu_population = pd.read_csv("jhu_countries_with_code.csv")
	...
	weather_station_list = pd.read_csv("ghcnd_stations.csv")[['id', 'state']]
	...
	weather_country_list = pd.read_csv("ghcnd_countries.csv")
	...
	weather_frame = weather_frame[['PRCP', 'TAVG']]
	...
	weather_frame = weather_frame[['date', 'country_region', 'state', 'PRCP', 'TAVG']]
	print(weather_frame)
	...
	weather_frame = weather_frame[['date', 'country_name', 'state', 'PRCP', 'TAVG']]
	...
	weather_future_pred = pd.read_csv("weather_model/future_pred.csv")
	...
	for country in list_country:
		...
		for state in list_state:
			...
			interpolate_feature_list = ['PRCP', 'TAVG']
